        ...
        <div class="form-container">
          ...
          >
            <div class="form-item">
              ...
              >
                <el-select
                  v-model="combinationForm[item.id]"
              ...
              <div
                v-else-if="item.mode == 'radio'"
                class="form-content"
              >
                <el-radio-group v-model="combinationForm[item.id]">
                  <el-radio
                    v-for="itemOptions in item.options"
                    :key="itemOptions.value"
                    :label="itemOptions.label"
...
export default {
  ...
  methods: {
    ...
    /**初始化，获取新增或编辑的状态，编辑时回显当前记录数据 */
    init() {
      this.solarParams = ports.solar;
      this.trackerParams = ports.tracker;
      this.combinationParams = ports.combination;
      const { activeStatus } = this;
      switch (activeStatus) {
        case 'add':
          this.cardTitle = 'New Integrated Data';
          break;
        case 'edit':
          this.cardTitle = 'Edit Integrated Data';
          break;
        default:
          break;
      }
