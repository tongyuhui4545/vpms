<template>
  <div>
    <div class="page-container">
      <!--团队人员分配-->
      <div
        v-if="showTeamAssignSection && show3DcalculationWorkflow"
      ...
      <!--发送progress-->
      <div
        v-if="show3DcalculationWorkflow && !showTeamAssignSection"
        class="drawer-container"
      >
        <el-form
          v-if="activeProgress['3d_calculation_status'] != 'CAL_US_PASSED'"
          ref="progressForm"
          :model="progressForm"
          label-width="120px"
          size="mini"
        ...
        >
          ...
          <el-form-item
            v-if="
              activeProgress['3d_calculation_status'] != 'CAL_CN_ASSIGNED' &&
                showCommentArea
            "
            label="Comment files"
          >
          ...
          <el-form-item label=" ">
            ...
            >
              <div
                v-if="
                  calculationStatus == 'CAL_CN_TEAM_ALREADY_APPLIED' &&
            ...
            >
              <div
                v-if="
                  calculationStatus == 'CAL_CN_PASSED' &&
...
export default {
  ...
  data() {
    return {
      ...
      uploadURL: '',
      fileList: [],
      show3DcalculationWorkflow: false,
      requestStatus: '',
      teamList: [],
  ...
  computed: {
    showCommentArea() {
      ...
      if (this.calculationStatus === 'CAL_CN_PASSED' && role === 'US Engineer') {
        return true;
      }
      //情况1，3D CN Engineer Manager 在他的操作中
      if (
        (this.calculationStatus === 'CAL_CN_TEAM_ALREADY_APPLIED' ||
          this.calculationStatus === 'CAL_US_HAS_BEEN_SEND') &&
        role === '3D Manager'
      ) {
        return true;
      }
      ...
      if (
        (this.calculationStatus === 'CAL_CN_ASSIGNED' ||
          this.calculationStatus === 'CAL_CN_REJECTED') &&
        role === '3D Team Leader'
      ) {
        return true;
      } else {return false;}
    ...
    underCNprocessing() {
      ...
      if (processingStatus.includes(this.calculationStatus)) {
        return true;
      } else {
        return false;
      }
    ...
    showTeamAssignSection() {
      if (
        ...
        (this.calculationStatus === 'CAL_US_HAS_BEEN_SEND' ||
          this.calculationStatus === 'CAL_US_REJECTED')
      ) {
        return true;
      }
      return false;
    ...
    showCNteamSubmit() {
      if (
        (this.calculationStatus === 'CAL_CN_ASSIGNED' ||
          this.calculationStatus === 'CAL_CN_REJECTED') &&
        !this.userLimits.approve_or_reject_team_submit &&
  ...
  watch: {
    progressDetailForProcess: {
      handler (val) {
        ...
        this.calculationStatus = this.activeProgress['3d_calculation_status'];
        if (val.phase === '60%') {
          this.d3Needed = true;
        }
        this.justifyRole();
  ...
  methods: {
    ...
    justifyRole() {
      ...
      this.show3DcalculationWorkflow = false;
    ...
    rejectProgress(state) {
      switch (state) {
        case 'CN_REJECTED':
          this.requestStatus = 'CAL_CN_REJECTED';
          this.rejectFormVisible = true;
          break;
        case 'US_REJECTED':
          this.requestStatus = 'CAL_US_REJECTED';
          this.rejectFormVisible = true;
          break;
        default:
          break
    ...
    proceedProcess() {
      const status = this.activeProgress['3d_calculation_status'];
      switch (status) {
        case 'CAL_US_HAS_BEEN_SEND':
          this.requestStatus = 'CAL_CN_ASSIGNED';
          break;
        case 'CAL_CN_ASSIGNED':
          this.requestStatus = 'CAL_CN_TEAM_ALREADY_APPLIED';
          break;
        case 'CAL_CN_TEAM_ALREADY_APPLIED':
          //要分情况判断，如果CN Manager拒绝了上传的文件，则请求状态为CAL_CN_REJECTED，否则为CAL_CN_PASSED
          if (!this.rejectForm.rejectReason) {
            this.requestStatus = 'CAL_CN_PASSED';
          }
          break;
        case 'CAL_CN_PASSED':
          //要分情况判断，如果US Engineer拒绝了上传的文件，则请求状态为CAL_US_REJECTED，否则为CAL_US_PASSED
          if (!this.rejectForm.rejectReason) {
            this.requestStatus = 'CAL_US_PASSED';
          }
          break;
        case 'CAL_CN_REJECTED':
          this.requestStatus = 'CAL_CN_TEAM_ALREADY_APPLIED';
          break;
        case 'CAL_US_REJECTED':
          this.requestStatus = 'CAL_CN_ASSIGNED';
          break;
        default:
          break
    ...
    changeProgress() {
      ...
      let type = '';
      if (this.processType === '3d-bidding') {
        type = 4;
      }
      if (this.processType === '3d-formal') {
        type = 5;
      }
      let params = {
        status: this.requestStatus,
        status_last: this.calculationStatus,
        project_id,
      ...
      if (this.requestStatus === 'CAL_CN_ASSIGNED') {
      ...
      await teamList({ type: '3D' })
        .then((res) => {
          if (res.code === 200) {
            ...
            // let status = activeProgress['3d_calculation_status'];
            // const that = this;
            // let activeTeam;
