<template>
  <div class="page-container">
    <Card class="card-templete">
      <template #filterArea>
        ...
        >
          <TableFilterV2
            v-if="showFilter"
            ref="tableFilterV2"
            filter-name="FormalProjectFilter"
            class="filter-area"
      ...
      <template #cardHeader>
        <span style="color: #303133; font-weight: 500; line-height: 20px">Formal</span>
      </template>
      <template #cardOperate>
        <div class="table-search">
          ...
          />
          <!-- <div v-if="userLimits.project_add" class="card-btn animate new-btn">
            ...
            >
              ...
              <template v-if="customizedTableColumns.includes('Customer')">
                ...
                >
                  ...
                  <!-- <template slot-scope="scope">
                <div v-if="scope.row.client_name">{{ scope.row.client_name }}</div>
              </template> -->
                </el-table-column>
              </template>
              <!-- Technical details -->
                      ...
                      <span
                        v-if="
                          scope.row.archiving == 0 &&
                            judgeDisplayClock(
                              'formal_project',
                              mapStatus(scope.row['3d_inner_date'])
                            )
                        "
                        style="margin-left: 3px"
                      >
                      ...
                      <span
                        v-if="
                          scope.row.archiving == 0 &&
                            judgeDisplayClock(
                              'formal_project',
                              mapStatus(scope.row['3d_team_due_date'])
                            )
                        "
                        style="margin-left: 3px"
                      >
              ...
              <!-- Simulation status -->
              <template
                v-if="customizedTableColumns.includes('Simulation status')"
              >
                <el-table-column
                  min-width="180"
                  :show-overflow-tooltip="true"
                >
                  <template slot="header">
                    ...
                    <Sorter
                      ref="simulation_status_label"
                      sorter-type="simulation_status_label"
                      sort-by="simulation_status_label"
                      @on-sort="handleSort"
                    />
                    <FastFilteredIcon
                      ref="fast_filtered_icon_simulation_status_label"
                      @cancel="handleCancelFastFilter"
                    />
                  </template>
                  <template slot-scope="scope">
                    <el-tooltip
                      :content="scope.row.simulation_status_label"
                      placement="top"
                    >
                      <QuoteStatusTag
                        :tag-text="scope.row.simulation_status_label"
                        type="formal"
                      />
                    </el-tooltip>
                  </template>
              ...
              <!-- Simulation team -->
              <template
                v-if="customizedTableColumns.includes('Simulation team')"
              >
                <el-table-column
                  prop="3d_team_name"
                  min-width="180"
                ...
                >
                  <template slot="header">
                    ...
                    >
                      ...
                      <FastFilter
                        fast-filter-type="SimulationTeam"
                        @fast-filter="handleFastFilter"
                      />
                    </el-popover>
                    <Sorter
                      ref="simulation_team"
                      sorter-type="simulation_team"
                      sort-by="team_id"
                      @on-sort="handleSort"
                    />
                    <FastFilteredIcon
                      ref="fast_filtered_icon_simulation_team"
                      @cancel="handleCancelFastFilter"
                    />
                  </template>
                </el-table-column>
              ...
              <!-- Simulation due date -->
              <template
                v-if="customizedTableColumns.includes('Simulation due date')"
              >
                <el-table-column min-width="160">
                  <template slot="header">
                    <el-popover
                      ref="simulationDueDatePopover"
                      placement="bottom-start"
                      width="250"
                      popper-class="el-popover-self"
                      trigger="hover"
                    ...
                    >
                      ...
                      <FastFilter
                        fast-filter-type="DateRangeFastFilter"
                        filter-for="simulation-due-date"
                        @fast-filter="handleFastFilter"
                      />
                    </el-popover>
                    <Sorter
                      ref="simulation_inner_date"
                      sorter-type="simulation_inner_date"
                      sort-by="simulation_inner_date"
                      @on-sort="handleSort"
                    />
                    <FastFilteredIcon
                      ref="fast_filtered_icon_simulation_inner_date"
                      @cancel="handleCancelFastFilter"
                    />
                  </template>
                  <template slot-scope="scope">
                    <span
                      ...
                      "
                    >
                      {{ scope.row.simulation_inner_date }}
                      <span
                        v-if="
                          scope.row.archiving == 0 &&
                            judgeDisplayClock(
                              'formal_project',
                              mapStatus(scope.row.simulation_inner_date)
                            )
                        "
                        style="margin-left: 3px"
                      >
                        <ClockTag
                          v-if="scope.row.simulation_due_date"
                          :due-date="scope.row.simulation_inner_date"
                        />
                      </span>
                    </span>
                  </template>
              ...
              <template
                v-if="
                  customizedTableColumns.includes('Simulation team due date')
                "
              >
                <el-table-column min-width="250">
                  <template slot="header">
                    <el-popover
                      ref="simulationTeamDueDatePopover"
                      placement="bottom-start"
                      width="240"
                      popper-class="el-popover-self"
                      trigger="hover"
                    ...
                    >
                      ...
                      <FastFilter
                        fast-filter-type="DateRangeFastFilter"
                        filter-for="simulation-team-due-date"
                        @fast-filter="handleFastFilter"
                      />
                    </el-popover>
                    <Sorter
                      ref="simulation_team_due_date"
                      sorter-type="simulation_team_due_date"
                      sort-by="simulation_team_due_date"
                      @on-sort="handleSort"
                    />
                    <FastFilteredIcon
                  ...
                  <template slot-scope="scope">
                    <span
                      ...
                      "
                    >
                      {{ scope.row.simulation_team_due_date }}
                      <span
                        v-if="
                          scope.row.archiving == 0 &&
                            judgeDisplayClock(
                              'formal_project',
                              mapStatus(scope.row.label_status)
                            )
                        "
                        style="margin-left: 3px"
                      >
                        <ClockTag
                          v-if="scope.row.simulation_team_due_date"
                          :due-date="scope.row.simulation_team_due_date"
                        />
                      </span>
                    </span>
                  </template>
              ...
              <!-- Label status -->
              <template v-if="customizedTableColumns.includes('Label status')">
                <el-table-column
                  prop="label_status"
                  min-width="180"
                  :show-overflow-tooltip="true"
                >
                  <template slot="header">
                    ...
                    />
                    <FastFilteredIcon
                      ref="fast_filtered_icon_label_status"
                      @cancel="handleCancelFastFilter"
                    />
                  </template>
                  <template slot-scope="scope">
                    <el-tooltip
                      :content="mapStatus(scope.row.label_status)"
                      placement="top"
                    >
                      <QuoteStatusTag
                        :tag-text="mapStatus(scope.row.label_status)"
                        type="formal"
                      />
                    </el-tooltip>
                  </template>
              ...
              <!-- Label team -->
              <template v-if="customizedTableColumns.includes('Label team')">
                <el-table-column min-width="120">
                  <template slot="header">
                    <el-popover
                      ref="labelTeamPopover"
                      placement="bottom-start"
                      width="130"
                      popper-class="el-popover-self"
                      trigger="hover"
              ...
              <template slot="header">
                <el-popover ref="labelSolutionPopover" placement="bottom-start" width="120" popper-class="el-popover-self"
                  trigger="hover" :visible-arrow="false">
                  <span slot="reference" class="column-on-hover" @click="closePopover">Label solution</span>
                  <FastFilter @fast-filter="handleFastFilter" fastFilterType="LabelSolution" />
                </el-popover>
                <Sorter sorterType="solution" sortBy="solution" @on-sort="handleSort" ref="solution" />
                <FastFilteredIcon ref="fast_filtered_icon_label_solution" @cancel="handleCancelFastFilter" />
              </template>
              <template slot-scope="scope">
                {{ scope.row.label_solution }}
              </template>
            </el-table-column> -->
              <!-- Label due date -->
              <template
                v-if="customizedTableColumns.includes('Label due date')"
              >
                <el-table-column
                  sort-by="due_date"
                  min-width="160"
                >
                  <template slot="header">
                    ...
                    />
                    <FastFilteredIcon
                      ref="fast_filtered_icon_label_due_date"
                      @cancel="handleCancelFastFilter"
                    />
                  </template>
                  <template slot-scope="scope">
                    ...
                    >
                      ...
                      >
                        <ClockTag
                          v-if="scope.row.label_due_date"
                          :due-date="scope.row.label_due_date"
                        />
                      </span>
                    </span>
                  </template>
...
import { mapStatus, judgeDisplayClock } from '@/utils/commons.js';
...
export default {
  ...
  //     //   vm.preventInitialList = true;
  //     //   vm.$refs.tableFilter.keepPage(vm.filterFor);
  //     // }
  //     if (clearFilterRouteNames.includes(from.name)) {
  //       vm.$store.dispatch('app/clearFilter');
  //     }
  //   });
  // },
  ...
  methods: {
    ...
    handleFastFilter(val, filterType, filterFor) {
      ...
      switch (filterFor) {
        ...
        case 'project_schedule_upload':
          ...
          break;
        case 'simulation-team-due-date':
          formData.simulation_team_due_date_start = val[0];
          formData.simulation_team_due_date_end = val[1];
          val = `${val[0]} ` + 'to' + ` ${val[1]}`;
          this.fastFilteredIconName =
            'fast_filtered_icon_simulation_team_due_date';
          break;
        case 'simulation-due-date':
          formData.simulation_inner_date_start = val[0];
          formData.simulation_inner_date_end = val[1];
          val = `${val[0]} ` + 'to' + ` ${val[1]}`;
          this.fastFilteredIconName =
            'fast_filtered_icon_simulation_inner_date';
          break;
      ...
      switch (filterType) {
        case 'SimulationTeam':
          formData.simulation_team_id = val.id;
          val = val.name;
          this.fastFilteredIconName = 'fast_filtered_icon_simulation_team';
          break;
        ...
        case 'SimulationStatus':
          formData.simulation_status = val.value;
          val = val.label;
          this.fastFilteredIconName =
            'fast_filtered_icon_simulation_status_label';
          break;
        case 'ProjectNo':
        ...
        case 'LabelStatus':
          formData.label_status = val.value;
          val = val.label;
          this.fastFilteredIconName = 'fast_filtered_icon_label_status';
          break;

        case 'LabelTeam':
          formData.label_team = val;
          val = val.name;
          this.fastFilteredIconName = 'fast_filtered_icon_label_team';
          break;

        case 'LabelDuedate':
          formData.label_due_date_start = val[0];
          formData.label_due_date_end = val[1];
          val = `${val[0]} ` + 'to' + ` ${val[1]}`;
          this.fastFilteredIconName = 'fast_filtered_icon_label_due_date';
          break;

        case 'LabelSolution':
          formData.label_solution = val;
    ...
    justifyRole() {
      ...
      if (limits.Bidding_Project_List.show_add_hours) {
    ...
    /**点击新增project按钮 */
    handleAdd() {
      this.$refs.addBiddingProjectModal.show();
    },

    /**点击查看project详情按钮 */
    handleView(row) {
      this.$router.push(`/index/project/formal/detail/overview/${row.id}`);
    },
    ...
    toFilter() {
      let params = {
        ...
        sales_status: this.filterFormData['sales_status'],
        ...
        creation_date_start: this.filterFormData.created_at_start,
        creation_date_end: this.filterFormData.created_at_end,
        ...
        label_status: this.filterFormData.label_status,
        label_team: this.filterFormData.label_team,
        label_solution: this.filterFormData.label_solution,
        label_due_date_start: this.filterFormData.label_due_date_start,
        label_due_date_end: this.filterFormData.label_due_date_end,
        '3d_team_id':
          this.filterFormData['3d_team_id'] ||
          this.filterFormData['simulation_team_id'],
        simulation_status: this.filterFormData['simulation_status'],
